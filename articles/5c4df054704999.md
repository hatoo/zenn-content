...
(set (edge 3 4) 3)
; EqSat実行
(run 3) ; 最大3回までイテレーションする。正直この数字はある程度大きければ何でもいいが文法上必要
; 1-4間の最短距離を出力
...
一見でとりあえずビームサーチすればどうにかなりそうに見えますが、この問題に対してはもっと良いアルゴリズム(Equality Saturation)とそれを可能にするデータ構造(e-graph)があります。
...
`e-graph`は上で言うS式をグラフにして保存します。グラフの表現力は無限大なので、仮にルールを適用していった結果情報が指数関数的に増えていったとしても`e-graph`はそれを効率的に表現できることがあります。
