...
このあとプロキシ側は本当に`example.com`に対するプロキシとして振る舞っても、別に適当にデータを作って返してもいいです。
...
別にRFCに書いてあるだけなので動けば別に気にしなくてもいいじゃないかと考えることもできるが、実際に https://www.google.com/ にHTTP/2のときに`Host`ヘッダーを送ると拒否されるので消す必要がある。
