...
Rust製のTLSライブラリ。`tokio-rustls`の`async`のときのブロッキングの仕方などが`native-tls`よりも良い感じな気がする。ただセキュリティ上問題のあるアルゴリズムがサポートされていないので、ブラウザからはアクセスできるがrustlsからはアクセスできないサイトがある。サーバー側で使う分には問題ない。
...
利用者は[`http::request::Request`](https://docs.rs/http/1.1.0/http/request/struct.Request.html)(URLがフルパス)を受け取って[http::response::Response](https://docs.rs/http/1.1.0/http/response/struct.Response.html)を返す[`HttpService`](https://docs.rs/hyper/latest/hyper/service/trait.HttpService.html)を実装することでプロキシのロジックを実装できます。
```rust
