...
4. `x`が`0`でない場合、`(/ x x)` を `1`に書き換える^[$x=0$の場合、$0/0=1$となってしまい、容易に誤った結論を導いてしまうので絶対にやってはいけない。[ゼロ除算#ゼロ除算に基づく誤謬](https://ja.wikipedia.org/wiki/%E3%82%BC%E3%83%AD%E9%99%A4%E7%AE%97#%E3%82%BC%E3%83%AD%E9%99%A4%E7%AE%97%E3%81%AB%E5%9F%BA%E3%81%A5%E3%81%8F%E8%AA%A4%E8%AC%AC)のようなことがそのまま起きる]
...
`egglog`にはジェネリクスのような機能がないので`set`、`map`、 `vec`は少し特殊になっています。[テストコード](https://github.com/egraphs-good/egglog/blob/main/tests/vec.egg)を見ると使い方がわかります。