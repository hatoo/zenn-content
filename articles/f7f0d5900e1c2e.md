...
ちなみにですが、`hyper`の`Service`は`tower`の`Service`とは直接の互換性はありません
https://github.com/hyperium/hyper/blob/0bd4adfed2bf40dda6867260ded05542d5b921c5/docs/ROADMAP.md?plain=1#L359-L368
...
利用者がリクエスト(URLがフルパス)を受け取ってレスポンスを返すHttpServiceを実装することでプロキシのロジックを実装できます。
```rust
