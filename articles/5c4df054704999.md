...
[このPR](https://github.com/egraphs-good/egglog/pull/249/files#diff-06ccb0b0ac8767ba02e52e9776bd9acfc212c87f8e943b06d093233c46b9e15a)でコードにわかりやすいドキュメントが追加されます🎉。幸い`egglog`はJITなどせずそのままASTを実行するので、具体的にどう実行されるかを読むのもそれほど難しくありません。
...
しかし、例えば`0`と`1`が同じ`Sort`になってしまうとすべてがめちゃくちゃになってしまうので組み込み型に`union`は使えません。
...
後述する`run`コマンドを実行するとS繰り返し`e-graph`に`rule`を適用していきます。
...
(rule
    ...
    ((union expr (Const (+ x y)))) ; Actionのリスト
)
...
ただ、現状可視化したときには関数名しか出てこないので、あまりおすすめしません。
