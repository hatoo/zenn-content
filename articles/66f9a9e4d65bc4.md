...
しかし、他のRustプロジェクトの[cloudflare/pingora](https://github.com/cloudflare/pingora/blob/main/docs/user_guide/panic.md)を見てみるとpanicくらいなら脆弱性とまではいかない感じがします。

# レポートを送る

`wasmtime`ではこのバグは脆弱性な気がしたためGitHubのSecurityタブからレポートを送りました。
